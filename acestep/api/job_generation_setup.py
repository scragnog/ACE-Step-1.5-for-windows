...
def build_generation_setup(
    *,
    ...
    params = GenerationParams(
        ...
        tempo_scale=getattr(req, "tempo_scale", 1.0),
        pitch_shift=getattr(req, "pitch_shift", 0),
        # PAG (Perturbed-Attention Guidance)
        use_pag=getattr(req, "use_pag", False),
        pag_start=getattr(req, "pag_start", 0.30),
        pag_end=getattr(req, "pag_end", 0.80),
        pag_scale=getattr(req, "pag_scale", 0.2),
        ...
        score_scale=getattr(req, "score_scale", 0.1),
        # Activation steering
        ...
        lm_repetition_penalty=getattr(req, "lm_repetition_penalty", 1.0),
