...
class _LoadSuccessBuilder:
    """Fake DatasetBuilder for load success flow."""

    def __init__(self) -> None:
        """Initialize fake metadata and deterministic sample list."""

        self.metadata = _Metadata()
        self.metadata.name = "loaded_name"
        self.samples = [_Sample()]

    def load_dataset(self, _dataset_path: str) -> tuple[list[_Sample], str]:
        """Return a successful load result."""

        return self.samples, "load-ok"

    def get_labeled_count(self) -> int:
        """Return deterministic labeled count for response payloads."""
...
class TrainApiDatasetScanLoadRoutesHttpTests(unittest.TestCase):
    ...
    def test_load_dataset_success_sets_dataset_json_path(self) -> None:
        """POST /v1/dataset/load should preserve the loaded dataset path for later incremental saves."""

        client = self._build_client()
        fake_module = types.SimpleNamespace(DatasetBuilder=_LoadSuccessBuilder)

        with mock.patch.dict(sys.modules, {"acestep.training.dataset_builder": fake_module}):
            response = client.post(
                "/v1/dataset/load",
                json={"dataset_path": "dir\\loaded.json"},
                headers={"Authorization": "Bearer test-token"},
            )
        ...
        payload = response.json()
        self.assertEqual(200, payload["code"])
        self.assertEqual("load-ok", payload["data"]["message"])
        self.assertEqual("dir/loaded.json", client.app.state.dataset_json_path.replace("\\", "/"))
